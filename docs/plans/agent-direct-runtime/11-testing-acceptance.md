...
| FE-24c | provider id가 있는 cold-restore metadata로 `resumeSession`/load를 시도했지만 provider 호출이 실패 | 실패한 controller를 shutdown/정리한 뒤 새 `startSession`으로 탭 컨텍스트를 유지하며, fallback modal 없이 "이전 대화를 복원할 수 없음" notice를 표시한다. 실패한 resume/load attempt에서 온 replay transcript event는 fresh start live store에 병합하지 않는다. fresh direct start 자체가 실패할 때만 §4.6 fallback 선택지를 표시한다(10 §4.4/§4.6). |
