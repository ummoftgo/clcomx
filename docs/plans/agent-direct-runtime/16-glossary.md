...
| **approval cleanup** | turn cancel / process exit 시 unresolved approval을 `cancelled`(또는 `failed`)로 닫고 wire에도 응답하는 불변식. ACP/Codex MUST와 대응. | [04](04-normalized-agent-model.md) §4.2, §5 |
