...
| OQ-52 | long-session transcript 윈도우/cap 수치 — `HOT_WINDOW`(보유 sealed turn 개수/byte), tombstone LRU 크기/TTL, 무거운 item(diff/이미지/출력) bounded 표현 한도 미확정 | 위험 §1.12, [`04-normalized-agent-model.md`](04-normalized-agent-model.md) §3.7, [`08-ui-composition.md`](08-ui-composition.md) §5, [`13-risks-open-questions.md`](13-risks-open-questions.md) §1.8, [`12-implementation-workstreams.md`](12-implementation-workstreams.md) T1.1·T1.3·T1.4 | **결정 필요(unverified)**: 보수적 기본값 = sealed-turn 윈도우 + 3-상태 residency(§1.12 완화책)를 유지하되 정확한 turn/byte cap·tombstone LRU/TTL·heavy-item 한도는 실측 전 미확정. image/data URI cap은 **OQ-12와 연동**(대용량 data URI 메모리 영향). | 대용량 세션 fixture(수백~수천 turn, 대형 diff/이미지 포함)로 heap·반응성 부하를 실측해 `HOT_WINDOW` turn/byte cap, tombstone LRU 크기/TTL, heavy-item bounded 표현(13 §1.8 ring/요약) 한도를 정하고 08 §5·04 §3.7·12 T1.1/T1.3/T1.4와 동기화. image cap은 OQ-12 저장 정책과 함께 확정. |
| OQ-53 | seal 후 same-turn 보조 notification 도착 여부 — 종료신호(Codex `turn/completed`·ACP `stopReason`) 뒤 같은 turn의 `tokenUsage`/`plan`/`diff`/늦은 `tool_call_update`가 실제 wire에서 오는가 | 위험 §1.12, [`04-normalized-agent-model.md`](04-normalized-agent-model.md) §3.7, [`05-codex-app-server-adapter.md`](05-codex-app-server-adapter.md), [`06-claude-acp-adapter.md`](06-claude-acp-adapter.md), [`11-testing-acceptance.md`](11-testing-acceptance.md) | **결정 필요(unverified)**: 보수적 기본값 = seal에 짧은 quiescence grace를 두고, 종료신호 후 도착하는 same-turn 보조 notification은 `sealed-retained` patch(unseal→patch→reseal+telemetry, 04 §3.7)로 흡수한다고 가정. 실제 도착 여부·종류·타이밍은 미검증. | 구현 직전 05/06 wire 실측(단일 turn 캡처)으로 `turn/completed`/`stopReason` 후 same-turn 보조 notification 도착 여부와 종류를 확인해 seal grace 길이·sealed-retained patch 범위를 04 §3.7에 확정한다. 11 late same-turn fixture는 **reducer/store event fixture**(adapter fixture 아님)로 흡수 경로를 고정하되, 실제 wire 확인은 이 OQ-53에서 한다. |
| OQ-54 | Codex `thread/read(includeTurns)` 복원 범위 — gap-only 복원인가 전체 snapshot인가(격리 replay 범위·비용) | 위험 §1.12, [`ref-codex-app-server-protocol.md`](ref-codex-app-server-protocol.md), [`ref-acp-protocol.md`](ref-acp-protocol.md) §3.4, [`10-persistence-migration.md`](10-persistence-migration.md) §4.7, [`12-implementation-workstreams.md`](12-implementation-workstreams.md) T5.6 | **결정 필요(unverified)**: tombstone 구간 격리 replay-reload(10 §4.7, T5.6)의 범위·비용은 `thread/read(includeTurns)`가 gap만 주는지 전체 snapshot을 주는지에 달려 있다. ACP `session/load`는 전체 재구성(ref-acp §3.4)으로 확인됨. Codex 측은 미검증. | 구현 직전 Codex app-server wire 실측으로 `thread/read(includeTurns)`가 gap-only인지 전체 snapshot인지 확인해 T5.6 격리 scratch replay 세션의 조회 범위·비용을 확정하고 10 §4.7과 동기화한다. 어느 쪽이든 격리 scratch 세션은 read-only이며 live store에 미병합(§1.12 완화 4). |
