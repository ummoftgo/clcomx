...
| OQ-47 | approval inline/modal 분류 신호 매핑 — 어떤 provider 신호를 `severity:"escalation"`(modal)로 올릴지 | 15 §5 `ApprovalRequest.severity`, [`08-ui-composition.md`](08-ui-composition.md) §4.4, [`05-codex-app-server-adapter.md`](05-codex-app-server-adapter.md)·[`06-claude-acp-adapter.md`](06-claude-acp-adapter.md) approval 매핑 | **결정 필요(v1 기본값)**: v1은 모든 approval을 `severity:"normal"`(inline)로 분류한다. provider escalation 신호(Codex sandbox 우회/`danger-full-access`, Claude `bypassPermissions`/`ExitPlanMode`)를 `"escalation"`(modal)로 올리는 매핑은 후속. | **구현 전 확인**: 05/06 approval 매핑에서 위 escalation 신호의 정확한 wire 표현을 확인한 뒤 `ApprovalRequest.severity`(15 §5) 값을 escalation으로 채우는 규칙을 확정하고 08 §4.4 inline/modal 렌더 분기와 합의. v1은 전부 normal 유지. |
