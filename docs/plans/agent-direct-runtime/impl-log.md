...
## 2026-07-03 구현 — OQ-16 하이브리드 cross-restart 복원

- v1은 앱 재시작 후 direct 대화 복원을 의도적으로 지원하지 않았다([`13-risks-open-questions.md`](13-risks-open-questions.md) OQ-16 원 결정, `workspace.json` scrub의 직접 결과). 이 슬라이스는 그 후속 설계([`oq16-cross-restart-restore-design.md`](oq16-cross-restart-restore-design.md), [`oq16-cross-restart-restore-plan.md`](oq16-cross-restart-restore-plan.md))를 구현해, scrub 경계(§6/TB-3)는 그대로 둔 채 cross-restart 하이브리드 복원(캐시 즉시 표시 + 가능하면 live resume)을 추가했다.
- **암호화 재개 id 저장소**(`src-tauri/src/features/agent_runtime/secret_store.rs`): OS 키스토어(`keyring` crate)의 단일 앱 키 1개(세션별 키 아님) + AES-256-GCM으로 세션별 `providerThreadId`/`providerSessionId`/`canResume`/`canLoad`를 at-rest 암호화 저장한다. `providerResumeToken`은 dead 필드로 확인되어 여전히 저장하지 않는다. command: `agent_runtime_save/load/clear_resume_keys`(`src-tauri/src/commands/agent_runtime.rs`). 키스토어 접근 실패·복호화 실패는 모두 `Ok(None)`으로 낮추는 graceful 폴백이다.
- **bounded transcript 캐시**: backend 파일 IO(`transcript_cache.rs`, 평문 JSON — 이미 redaction 통과분만 담으므로 암호화 불필요) + frontend 직렬화(`service/transcript-cache.ts`, `TranscriptModel` Map↔array 변환, residency 윈도우 bound, tombstone 제외, display-redaction 적용). command: `agent_runtime_save/load/clear_transcript_cache`. 저장은 디바운스해 IO 빈도를 낮췄다.
- **하이브리드 복원 흐름**(`AgentTranscriptSurface.svelte`): 재시작 시 캐시가 있으면 `store.hydrateReadOnly`로 **즉시 read-only 렌더**한다. 실제 resume/process spawn은 **탭이 처음 보일 때**(`props.visible` 최초 `true`, 정확히 1회)로 **지연**해 다중 세션 동시 복원 AppHang을 피한다(예약된 후속 AppHang 수정 작업과 지연 원칙 공유). 재개 id는 `workspace.json`의 scrub된 metadata가 아니라 `loadResumeKeys`(암호화 저장소)에서 주입한다.
- **리컨실리에이션**: resume이 **권위 replay로 성공**하면 `store.discardReadOnlyHydration()`으로 캐시를 비우고 replay로 재구성해 중복이 없다. resume이 **미지원**(`canResume==false && canLoad==false`)이거나 재개 id가 없는데 캐시는 있으면 `historyReadOnly` notice로 캐시를 read-only 히스토리로 유지한다 — 단 이 notice는 composer를 잠그지 않는다. 그 아래에서 **fresh 세션이 정상적으로 시작**되고, 시작이 끝나면 composer가 활성화되어 바로 이어 입력할 수 있다("이어가려면 새 세션"의 실제 동작). resume **RPC가 실패**하면 기존 v1 fallback(실패 controller 정리 → fresh start + `restoreUnavailable` notice, [`10-persistence-migration.md`](10-persistence-migration.md) §4.4)을 그대로 따른다.
- **탭 삭제 GC**: 사용자가 direct 세션 탭을 **명시적으로 닫을 때만**(`App.svelte::handleCloseTab` → `session-lifecycle-controller.ts`) 암호화 재개 id(`clearResumeKeys`)와 transcript 캐시(`clearTranscriptCache`)를 정리한다. **앱 종료 경로**(`window-close-orchestration-controller.ts`)는 반대로 `captureResumeIdsBeforeAppClose`로 재개 id를 보존해 다음 cold restore에 대비한다 — teardown(webview reload/컴포넌트 destroy)에서는 GC하지 않는다.
- GREEN: `cargo test --manifest-path src-tauri/Cargo.toml`의 `secret_store`/`transcript_cache` 신규 단위 테스트(암·복호화 왕복, 손상 blob graceful `Ok(None)`, 캐시 save/load/clear round-trip) 통과. `npx vitest run src/lib/features/agent-runtime/`의 신규 `transcript-cache`/`resume-store`/`AgentTranscriptSurface` 하이브리드 복원 테스트 통과.
- **E2E**: `e2e/agent-runtime/agent-runtime.test.ts`에 E2E-13a(암호화 재개 id + 캐시 시드 → 캐시 즉시 렌더 + mock provider resume 성공 이어가기)와 E2E-13b(resume 양쪽 미지원 시드 → read-only 히스토리 표시, fresh 세션 시작 후 composer 사용 가능)를 추가했다. (작성 시점에는 Windows 러너 미실행이었으나 **2026-07-05 실행·통과** — 아래 2026-07-05 섹션 참조.) `secret_store.rs`의 실제 OS 키스토어를 요구하는 테스트(`save_load_clear_resume_keys_by_handle`, `app_key_is_32_bytes_and_stable_across_calls`)는 `#[ignore]`로 표시돼 있고 이 WSL/Linux 개발 환경에서는 실행되지 않는다 — 실 키스토어 검증은 로컬/Windows에서 별도 확인이 필요하다.
- 정본 갱신: [`10-persistence-migration.md`](10-persistence-migration.md) §4.4/§4.4a(신규)/§4.5, [`13-risks-open-questions.md`](13-risks-open-questions.md) OQ-16, [`09-permissions-security.md`](09-permissions-security.md) §1 TB-5(신규)/§7.1(신규)를 이 구현에 맞춰 갱신했다(Task 12, 문서 전용 커밋).

## 2026-07-05 후속 — OQ-16 리뷰 후속 수정 + AppHang PTY lazy spawn + Windows E2E 실행

- **폴백 GC**(`fd83f7a`): direct→PTY 폴백(`handleFallbackToPty`)이 raw closeSession으로 GC를 우회해 이전 실행의 재개 키/캐시가 영구 고아가 되던 갭 수정 — lifecycle 컨트롤러 `handleFallbackToPtyClose`(close+GC)로 위임. GC 허용 경로는 "명시적으로 버리기" 두 곳(탭 닫기·폴백)이 정본(10 §4.4a 갱신, 위 2026-07-03 섹션의 "닫을 때만" 서술은 이 시점으로 대체됨).
- **resume 실패 캐시 손실(flicker) 수정**(`28fea3f`): 실패 catch의 `controller.dispose()` → `store.dispose()`가 transcript를 통째로 비워 read-only 캐시가 replay 유무와 무관하게 소실되던 버그. start 전에 캐시 모델을 보관했다가 dispose 후 재주입해 `historyReadOnly`로 복귀(캐시 없으면 기존 `restoreUnavailable`). 위 2026-07-03 섹션의 "resume RPC 실패는 v1 fallback 그대로" 서술은 캐시-보유 시 read-only 복귀로 대체됨(10 §4.4a 정본).
- **다중 세션 동시 복원 AppHang — PTY lazy spawn**(`39926b6..8394392`, Codex adversarial 5라운드 APPROVE): cold PTY spawn(CLI resume 내장)을 첫 visible로 지연, alive attach는 eager(spawn 폴백 금지 + dispose 경합 가드/고아 회수), 에디터 hydration은 metadata eager/content lazy(사용자 편집 보존 병합). Windows 3탭 실재현 검증은 남은 작업.
- **Windows E2E 실행**(`095d061`, `3534e84`): agent-runtime pack **20/20 통과**(E2E-13a/13b 첫 실행 포함). E2E-13a의 원 seed 전제(workspace.json provider id 폴백)는 읽기 경로 방어 scrub 때문에 성립 불가로 판명 — test-mode 고정 앱 키(debug 빌드 한정 `#[cfg(debug_assertions)]`, release 컴파일 제거)로 암호화 저장소 파일을 직접 시드하는 방식으로 교체해 TB-5 복호화 → `thread/resume` 경로를 실검증한다. 같은 키로 저장/로드/삭제 왕복 Rust 테스트가 CI에서도 돈다(Rust 175 pass).
