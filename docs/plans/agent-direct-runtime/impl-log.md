...
- **E2E**: `e2e/agent-runtime/agent-runtime.test.ts`에 E2E-13a(암호화 재개 id + 캐시 시드 → 캐시 즉시 렌더 + mock provider resume 성공 이어가기)와 E2E-13b(resume 양쪽 미지원 시드 → read-only 히스토리 표시, fresh 세션 시작 후 composer 사용 가능)를 추가했다. **이 두 spec은 Windows E2E 러너에서는 아직 실행하지 않았다**(이 개발 환경 제약 — 실제 Windows app-boundary 확인은 남은 작업). `secret_store.rs`의 실제 OS 키스토어를 요구하는 테스트(`save_load_clear_resume_keys_by_handle`, `app_key_is_32_bytes_and_stable_across_calls`)도 같은 이유로 `#[ignore]`로 표시돼 있고 이 WSL/Linux 개발 환경에서는 실행되지 않는다 — 로컬/Windows에서 별도 확인이 필요하다.
