...
> - **위험 비은닉(현재 권위 ∪ 다음 turn intent)**: 고위험 표시는 override-우선 "실효 정책"이 아니라 **현재 권위**(현재 thread 정책 = `runtimeMetadata.approvalPolicy`)와 **다음 turn intent**(사용자 override)를 **분리해 둘 중 하나라도 위험하면** 세운다(자세한 불변식은 아래 "위험 판정" 항목). 현재 권위가 `never`(또는 미상/granular/미인식)이면 override 없이도, 그리고 사용자가 안전 scalar를 골라도 현재 권위의 고위험을 유지한다(안전 override는 다음 turn intent일 뿐 현재 wire를 바꾸지 않는다). 다음 turn intent가 `never`/sentinel이면 그 자체로 고위험이다. 사용자가 건 override는 현재 권위 badge를 그대로 둔 채 **정보성 override chip**("다음 turn: {값}")으로만 노출하고, 고위험 표시를 안전 override 선택만으로 조기 해제하지 않는다 — 현재 권위 위험은 서버 `thread/settings/updated` echo가 known safe scalar로 base를 갱신해야 풀린다.
...
> - **권위 settings echo 반영**: 서버는 `thread/settings/updated` 알림(`ThreadSettings`: approvalPolicy/sandboxPolicy/model/effort)으로 권위 설정을 다시 알린다 — 우리 `turn/start` override 적용 후, provider default 정규화, 또는 타 클라이언트 변경 시. Codex mapper가 이를 `runtime_metadata_changed`로 매핑해 `runtimeMetadata`의 approval/sandbox를 갱신한다 — 이 값이 위 불변식의 **현재 권위**이므로, 서버가 `never`로 바꾸면 UI가 즉시 고위험으로 갱신되고, override 적용을 안전 scalar로 echo하면 현재 권위 위험이 풀린다(Codex medium 10차). settings/updated는 **full snapshot**이라 approvalPolicy·sandbox를 항상 반영하며, 인식 불가(future/malformed) 값이면 approvalPolicy는 `undefined`로 clear(→ known scalar 아님 → 고위험), sandbox는 `SANDBOX_UNKNOWN`으로 표시해(→ 고위험) 이전 안전값이 남지 않는다(Codex medium 17·18·20차). model/effort는 셀렉터가 별도 상태라 이 echo로 갱신하지 않는다(selector 재조정은 ②-C 범위 밖 후속). 겹친 submit 중 하나가 이미 성공해 active turn이 있을 때 다른 `turn/start` 실패는 `ready`로 되돌리지 않아(active turn 확인) running 중 셀렉터/stop이 잘못 풀리지 않는다(Codex high 17차). composer footer는 옵션 popover와 별개로 sandbox/permission indicator를 계속 노출하고 고위험 값은 강조를 유지한다.
> - **위험 판정 = 현재 권위 ∪ 다음 turn intent(핵심 불변식)**: `setTurnOptions`는 다음 turn/start에 실릴 override intent만 저장하고, 현재 thread 권위 정책(`runtimeMetadata.approvalPolicy`)은 서버 echo 전까지 그대로다. 따라서 approval 고위험은 **현재 권위 위험**(base가 `never`이거나 known scalar로 확인 불가=granular/replay 미상/미인식)과 **다음 turn intent 위험**(`never` scalar override 또는 provider 기본값 sentinel)을 **분리해 둘 중 하나라도 위험하면** 고위험으로 표시한다(Codex high 21차). 즉 현재 권위가 `never`인데 사용자가 안전 scalar를 골라도 그 안전값은 다음 turn intent일 뿐이므로 현재 권위 `never`의 고위험을 축소 표시하지 않는다 — 서버 `thread/settings/updated`가 override 적용을 known safe scalar로 echo해 base가 갱신돼야 비로소 풀린다. 안전 override는 override chip("다음 turn: …")으로만 노출한다.
> - **sentinel은 상시 fail-closed(상관 없음)**: provider 기본값 sentinel은 adapter가 **매 turn `approvalPolicy:null`을 재전송**(turnApprovalPolicy=null 지속)하므로 다음 turn 정책이 항상 provider default(결과 미상)다. 따라서 sentinel은 echo/turn과 **상관해 해소하지 않고**, 선택되어 있는 동안 계속 fail-closed 고위험으로 둔다. 미상을 벗어나려면 사용자가 **명시 scalar를 선택**해 다음 turn을 known scalar로 확정해야 한다(그때 sentinel intent가 해소된다; 현재 권위 위험은 별도로 echo로 풀린다). 이 단순 불변식은 "null이 wire에 커밋된 뒤의 echo만 신뢰"하려던 이전 상관 모델(및 `onTurnStarted`/커밋 추적)을 대체한다 — 상관 식별자 부재로 조기 해소 위험이 있어 제거했다. 셀렉터는 ready/idle에서만 열리고 prompt submit in-flight 동안 잠가(pending count) 옵션 변경이 in-flight turn과 경합하지 않게 한다(UX 안정).
> - **미상 정책 fail-closed**: replay resume은 `thread/read`로 재개하는데 `ThreadReadResponse`는 `approvalPolicy`를 담지 않는다(`thread/resume`은 담음). 따라서 replay로 복원한 thread의 실제 정책이 `never`여도 client는 알 수 없다. Codex 세션에서 현재 권위 정책을 **known scalar**(untrusted/on-failure/on-request/never)로 확인하지 못하면(미상 또는 granular) 안전한 상태로 보이지 않게 **fail-closed로 고위험 취급**한다 — metadata approval 항목을 `data-risk="high"`로, 옵션 토글을 고위험으로 표시한다(Codex high 재지적: replay never 은닉 방지). 서버가 이후 `thread/settings/updated`로 known scalar를 알리면 현재 권위 위험이 풀린다.
...
증거: `codex-app-server-adapter.test.ts`의 ②-C 계열(never override wire 전송, 미설정 시 미포함, null revert, model/effort/approval no-clobber, 겹친 submit active turn 시 turn/start 실패가 ready 미emit), `codex-wire-mapper.test.ts`의 `thread/settings/updated`→metadata 매핑(approval/sandbox 항상 clear, 미인식 approval undefined·미인식 sandbox `SANDBOX_UNKNOWN`), `AgentComposer.test.ts`의 approval 셀렉터/never·sentinel 확인 게이트(accept/cancel/readiness·cross-pending·포커스)/popover·footer indicator 계열, `AgentTranscriptSurface.test.ts`의 위험 판정 계열(현재 권위 never면 안전 override로도 고위험 유지→echo로 해소, base never/미상/미인식 sandbox fail-closed, sentinel은 echo/turn 무관 상시 고위험·명시 scalar로만 해소, in-flight/겹친 submit 셀렉터 잠금).
