...
/**
 * Codex `AskForApproval` scalar 후보(②-C approval override 셀렉터 소스). granular 객체 variant는
 * experimental이라 후보/override wire에서 제외한다(표시만 — buildCodexPolicyMetadata가 "granular"로 축약).
 * 순서는 위험도 오름차순(untrusted→never)이 아니라 wire enum 순서를 따른다.
 */
 ...
 */
export function isHighRiskApprovalPolicy(policyId: string | undefined): boolean {
  return policyId === "never";
}
